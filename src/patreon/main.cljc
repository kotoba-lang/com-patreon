(ns patreon.main
  "Kotodama WASM entrypoint for the Patreon clean-room actor (L4) — Clojure port.
  ...
  facts (`patreon.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "patreon")
(def tier "L4")
...
(def entity-specs
  [{:entity "User"     :plural "users"     :id-prefix "patreon_use"
    :fields [:about :fullName :firstName :lastName :email :isCreator :isEmailVerified :likeCount :created] :required [:about]
    :coerce {:isCreator :bool :isEmailVerified :bool :likeCount :int} :refs {}}
   {:entity "Member"   :plural "members"   :id-prefix "patreon_mem"
    :fields [:patronStatus :lastChargeStatus :fullName :email :currentlyEntitledAmountCents :campaignLifetimeSupportCents :willPayAmountCents :isFollower :isFreeTrial :lastChargeDate :nextChargeDate :pledgeRelationshipStart] :required [:patronStatus]
    :coerce {:currentlyEntitledAmountCents :int :campaignLifetimeSupportCents :int :willPayAmountCents :int :isFollower :bool :isFreeTrial :bool} :refs {}}
   {:entity "Campaign" :plural "campaigns" :id-prefix "patreon_cam"
    :fields [:creationName :currency :name :patronCount :payPerName :isMonthly :isNsfw :publishedAt] :required [:creationName]
    :coerce {:patronCount :int :isMonthly :bool :isNsfw :bool} :refs {}}
   {:entity "Tier"     :plural "tiers"     :id-prefix "patreon_tie"
    :fields [:amountCents :description :patronCount :published :requiresShipping :title] :required [:amountCents]
    :coerce {:amountCents :int :patronCount :int :published :bool :requiresShipping :bool} :refs {}}
   {:entity "Benefit"  :plural "benefits"  :id-prefix "patreon_ben"
    :fields [:benefitType :description :isDeleted :isPublished :title :tiersCount] :required [:benefitType]
    :coerce {:isDeleted :bool :isPublished :bool :tiersCount :int} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"patreon.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "patreon-compat" :tier tier :entities entities} 200])
...
(defn start! [] :patreon-compat/ready)
